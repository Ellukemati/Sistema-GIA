...
impl ReservaHandler {
    ...
    pub fn mostrar_formulario_reserva() -> Response {
        ...
        let html =
            include_str!("../../templates/reserva.html");
    ...
    pub fn procesar_reserva(
        ...
        let usuario_id = match datos
            .get("usuario_id")
            .and_then(|v| v.parse::<i64>().ok())
        {
            Some(id) => id,
            None => {
                return Response::text("Usuario inválido")
                    .with_status_code(400);
            }
        };

        let ejemplar_id = match datos
            .get("ejemplar_id")
            .and_then(|v| v.parse::<i64>().ok())
        {
            Some(id) => id,
            None => {
                return Response::text("Ejemplar inválido")
                    .with_status_code(400);
            }
        };
    ...
    fn parsear_formulario(
        ...
        for par in body.split('&') {
            ...
            if let (Some(clave), Some(valor)) =
                (partes.next(), partes.next())
            {
