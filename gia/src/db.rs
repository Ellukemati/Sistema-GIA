...
pub fn init_db(db_path: &str) -> SqlResult<Connection> {
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS modelos_instrumentos (
            ...
            descripcion TEXT,
            manual_url TEXT,
            imagen_principal_direccion TEXT
        )",
        [],
    )?;
...
pub fn insert_modelo_instrumento(
    ...
    categoria: &str,
    manual_url: &str,
    imagenes: &[String], // Puede estar vacio. Si hay imagenes, la primera se asigna a imagen_principal_direccion
) -> Result<(), DbError> {
    ...
    for (i, ruta) in imagenes.iter().enumerate() {
        tx.execute(
            "INSERT INTO modelo_imagen (modelo_id, orden, imagen_direccion)
             VALUES (?, ?, ?)",
...
pub fn update_modelo_instrumento(
    ...
    // Si se mantienen imagenes previas, deben ser reenviadas por el frontend para su reasignacion
    let imagenes_previas = {
        let mut stmt =
            tx.prepare("SELECT imagen_direccion FROM modelo_imagen WHERE modelo_id = ?1")?;
    ...
    let nuevas_direcciones: HashSet<&str> = data
        .nuevas_imagenes
        .iter()
